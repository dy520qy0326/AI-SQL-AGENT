...
export function DiffView() {
  ...
  if (error || !diff) {
    return (
      <div>
        <Link to={`/projects/${id}/versions`} className="mb-4 inline-flex items-center gap-1 text-sm text-gray-500 no-underline hover:text-gray-700">
      ...
      <div className="flex flex-wrap gap-3">
        {(dd.tables_added?.length > 0 || dd.tables_removed?.length > 0) && (
          <div className="flex items-center gap-1 rounded-lg border bg-white px-3 py-2 text-sm">
            ...
            <span className="text-gray-500 ml-1">表</span>
          </div>
        )}
        {(dd.fields_added?.length > 0 || dd.fields_removed?.length > 0 || dd.fields_modified?.length > 0) && (
          <div className="flex items-center gap-1 rounded-lg border bg-white px-3 py-2 text-sm">
            ...
            <span className="text-gray-500 ml-1">字段</span>
          </div>
        )}
        {(dd.indexes_added?.length > 0 || dd.indexes_removed?.length > 0) && (
          <div className="flex items-center gap-1 rounded-lg border bg-white px-3 py-2 text-sm">
            ...
            <span className="text-gray-500 ml-1">索引</span>
          </div>
        )}
        {dd.breaking_changes && (
          <div className="flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm">
            <AlertTriangle className="h-4 w-4 text-red-500" />
            <span className="text-red-700 font-medium">{dd.breaking_details?.length || 0}</span>
            <span className="text-red-600">项破坏性变更</span>
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg border bg-white">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          {diff.summary && (
            ...
            >
              {summaryCopied ? <Check className="h-3 w-3 text-green-600" /> : <Copy className="h-3 w-3" />}
        ...
        <div className="px-4 py-3 text-sm text-gray-700">
          ...
          {!diff.summary && !aiSummary.isPending && (
            <p className="text-gray-400">点击"生成摘要"由 AI 分析本次变更</p>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          {migration.data && (
            <button
              ...
              className="flex items-center gap-1 rounded-md border px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50 cursor-pointer"
            >
              {sqlCopied ? <Check className="h-3 w-3 text-green-600" /> : <Copy className="h-3 w-3" />}
              {sqlCopied ? '已复制' : '复制'}
            </button>
          )}
        </div>
        <div className="p-4">
          {!migration.data && !migration.isPending && (
            <button
              ...
              className="flex items-center gap-1 rounded-md border px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 cursor-pointer disabled:opacity-50"
            >
